"""Seed reproducible de UN proyecto demo CENSOPAS-COPSOQ (versión media),
con las convenciones actuales del Punto 4/5 de esta sesión — no reutiliza
la siembra de plan preventivo/BSC de `seed_censopas_demo.py` (ese script
queda intacto, sin tocar, como referencia histórica).

Diferencias deliberadas frente a `seed_censopas_demo.py`:
  - Un solo proyecto (versión MEDIA), no dos — para tener un caso de uso
    completo y cerrado en vez de dos a medias.
  - `ActionPlanItem` usa los campos nuevos: `origin_hypothesis` (separado de
    `finding`/`action_description`) y `analysis_run_id` (apunta al
    AnalysisRun canónico real que originó la acción, no a `None`).
  - Estados mixtos deliberados (PENDING/IN_PROGRESS/DONE y uno vencido de
    verdad respecto a la fecha real del sistema) para que `effective_status`
    muestre los cuatro casos en el panel de resumen del Plan Preventivo.
  - Historial de KPI más largo (6 mediciones mensuales por indicador, no 3)
    para que la evolución se vea en el detalle de seguimiento.
  - El estudio queda **CLOSED** al final (el script viejo lo dejaba abierto)
    — "cerrado por completo": dashboard, instrumentos, telemetría, BSC y
    reporte final ya generados y persistidos antes de cerrar.

Reutiliza del script viejo únicamente lo que es contenido real del manual
CENSOPAS-COPSOQ (catálogo de dimensiones/subdimensiones/ítems/escalas) y la
infraestructura de siembra de respuestas — no la parte de BSC/timeline, que
se reescribió aquí.

Uso (desde backend/, con el venv activo):
    python scripts/seed_demo_current.py
"""
...
SHORT_VERSION_ID = 1
MEDIUM_VERSION_ID = 2
# Este script solo siembra la versión MEDIA (un proyecto, cerrado por
# completo). DEMO_CODE_SHORT se conserva únicamente porque `reset_demo` y
# las funciones `seed_demo_short`/`seed_plan_and_bsc_short` heredadas (no
# usadas por `run()` en este script) todavía lo referencian.
DEMO_CODE_SHORT = "SYN-C20-2026"
DEMO_CODE_MEDIUM = "DEMO-CURRENT-M200-2026"
SEED_VERSION = "CENSOPAS_DEMO_CURRENT_V1"
...
async def seed_demo_medium(session: AsyncSession, owner: User, build: ManifestBuild) -> dict:
    ...
    analysis_run, summary = await run_canonical_scoring(session, study.id)
    print(f"[{DEMO_CODE_MEDIUM}] scoring canónico: {summary.model_dump()}")

    await seed_plan_and_bsc_medium(session, study, analysis_run_id=analysis_run.id)
    await seed_analytics_exports_reports(
        session,
        owner,
        project,
    ...
    # "Cerrado por completo" (Punto 3): el estudio queda CLOSED una vez que
    # dashboard/instrumentos/telemetría/BSC/reporte ya están persistidos —
    # el script viejo lo dejaba abierto indefinidamente.
    study = await StudyService(session).close(study.id)
    print(f"[{DEMO_CODE_MEDIUM}] estudio cerrado: status={study.status}")

    return {"project_id": project.id, "study_id": study.id, "survey_id": survey.id}


async def seed_plan_and_bsc_medium(session: AsyncSession, study: Study, *, analysis_run_id: int) -> None:
    """Plan preventivo con las convenciones del Punto 4 de esta sesión:
    `finding`/`origin_hypothesis`/`action_description` separados (nunca
    fusionados), `analysis_run_id` apuntando al AnalysisRun canónico real
    que originó cada acción vinculada a un constructo (None para la medida
    organizacional genérica sin constructo), y estados mixtos deliberados
    para ejercitar los cuatro casos de `effective_status`
    (PENDING/IN_PROGRESS/DONE/OVERDUE — este último derivado de una fecha
    de vencimiento real ya pasada, no hardcodeado como string)."""
    d1 = (
    ...
    plan = ActionPlan(study_id=study.id, name=f"Plan preventivo — {DEMO_CODE_MEDIUM}", status="ACTIVE")
    session.add(plan)
    await session.flush()
    ...
    items = [
        ActionPlanItem(
            ...
            origin_hypothesis="Carga de trabajo elevada, plazos cortos e interrupciones frecuentes en el área Operaciones.",
            action_description="Rebalancear cargas semanales en Operaciones y revisar turnos rotativos.",
            responsible_label="Jefatura de Operaciones",
            priority=1,
            due_date=date(2026, 9, 24),  # futuro respecto a hoy -> IN_PROGRESS se mantiene
            status="IN_PROGRESS",
        ),
        ActionPlanItem(
            action_plan_id=plan.id,
            ...
            origin_hypothesis="Metas de producción por turno posiblemente desalineadas con la dotación actual de personal.",
            action_description="Revisar dotación de personal y metas de producción por turno.",
            responsible_label="Jefatura de Operaciones",
            priority=2,
            due_date=date(2026, 7, 20),  # pasado respecto a hoy -> effective_status = OVERDUE
            status="PENDING",
        ),
        ActionPlanItem(
            action_plan_id=plan.id,
            ...
            finding="Preocupación elevada por estabilidad contractual reportada en S17 (sintético).",
            origin_hypothesis="Renovaciones de contrato recientes sin comunicación formal previa al personal.",
            action_description="Sesión informativa sobre estabilidad contractual con RR. HH. — realizada y verificada.",
            responsible_label="SST y RR. HH.",
            priority=3,
            due_date=date(2026, 6, 20),  # pasado, pero DONE nunca se convierte en OVERDUE
            status="DONE",
        ),
        ActionPlanItem(
            action_plan_id=plan.id,
            construct_id=None,
            analysis_run_id=None,  # medida organizacional genérica, sin constructo asociado
            title="Comunicación de resultados por sede (demo)",
            finding="Pendiente de difundir resultados agregados a las jefaturas de sede.",
            origin_hypothesis=None,
            action_description="Difundir resultados agregados respetando n>=5 y supresión secundaria.",
            responsible_label="Comité SST",
            priority=4,
            due_date=date(2026, 11, 1),  # futuro -> PENDING se mantiene
            status="PENDING",
        ),
    ]
    session.add_all(items)
    ...
    # Historial más largo (6 mediciones mensuales por KPI, no 3) para que la
    # evolución se vea con detalle en el seguimiento del Plan Preventivo.
    measurements = []
    base_date = date(2026, 3, 15)
    kpi_series = [
        [80.0, 82.5, 84.0, 86.5, 88.0, 89.5],
        [58.0, 61.0, 64.5, 67.0, 69.0, 71.5],
    ]
    for kpi, values in zip(kpis, kpi_series):
        for i, value in enumerate(values):
            ...
            measurements.append(
                KpiMeasurement(
                    kpi_id=kpi.id,
                    measured_at=datetime(year, month, min(base_date.day, 28), tzinfo=UTC),
                    numeric_value=value,
